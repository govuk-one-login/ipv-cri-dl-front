...
exports.DrivingLicencePage = class PlaywrightDevPage {
  ...
  async assertLastName(dvlaLastNameWelsh) {
    await this.page.waitForLoadState("domcontentloaded");
    expect(await this.isCurrentPage()).to.be.true;
    await expect(await this.lastNameWelsh.textContent()).to.contains(
      dvlaLastNameWelsh
    );
  }
  ...
  async assertGivenName(dvlaGivenNameWelsh) {
    await this.page.waitForLoadState("domcontentloaded");
    expect(await this.isCurrentPage()).to.be.true;
    expect(await this.givenNameWelsh.textContent()).to.contain(
      dvlaGivenNameWelsh
    );
  }
  ...
  async assertMiddleName(dvlaMiddleNameWelsh) {
    await this.page.waitForLoadState("domcontentloaded");
    expect(await this.isCurrentPage()).to.be.true;
    expect(await this.middleNames.textContent()).to.contain(
      dvlaMiddleNameWelsh
    );
  }
  ...
  async assertDobExample(dobExampleWelsh) {
    await this.page.waitForLoadState("domcontentloaded");
    expect(await this.isCurrentPage()).to.be.true;
    expect(await this.dobExample.innerText()).to.equal(dobExampleWelsh);
  }

  async assertDayWelsh(dayWelsh) {
    await this.page.waitForLoadState("domcontentloaded");
    expect(await this.isCurrentPage()).to.be.true;
    await expect(await this.dayInWelsh.innerText()).to.equal(dayWelsh);
  }

  async assertMonthWelsh(monthWelsh) {
    await this.page.waitForLoadState("domcontentloaded");
    expect(await this.isCurrentPage()).to.be.true;
    await expect(await this.monthInWelsh.innerText()).to.equal(monthWelsh);
  }

  async assertYearWelsh(yearWelsh) {
    await this.page.waitForLoadState("domcontentloaded");
    expect(await this.isCurrentPage()).to.be.true;
    await expect(await this.yearInWelsh.innerText()).to.equal(yearWelsh);
  }
  ...
  async assertIssueDayWelsh(issueDayWelsh) {
    await this.page.waitForLoadState("domcontentloaded");
    expect(await this.isCurrentPage()).to.be.true;
    expect(await this.issueDayInWelsh.innerText()).to.equal(issueDayWelsh);
  }

  async assertIssueMonthWelsh(issueMonthWelsh) {
    await this.page.waitForLoadState("domcontentloaded");
    expect(await this.isCurrentPage()).to.be.true;
    expect(await this.issueMonthInWelsh.innerText()).to.equal(issueMonthWelsh);
  }

  async assertIssueYearWelsh(issueYearWelsh) {
    await this.page.waitForLoadState("domcontentloaded");
    expect(await this.isCurrentPage()).to.be.true;
    expect(await this.issueYearInWelsh.innerText()).to.equal(issueYearWelsh);
  }

  async assertValidToDayWelsh(validToDayWelsh) {
    await this.page.waitForLoadState("domcontentloaded");
    expect(await this.isCurrentPage()).to.be.true;
    expect(await this.validToDayInWelsh.innerText()).to.equal(validToDayWelsh);
  }
